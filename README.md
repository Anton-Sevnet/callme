...
### 7. **Fail-safe скрытие карточек в очередях и группах**

- Для каждой ноги очереди/ring-group CallMe теперь отслеживает `HangupEvent` по `agent_uniqueid`. Если Asterisk 1.8 не отдал `CallMeRingingStop`, PHP-демон сам вызывает `callme_force_ring_entry_cleanup()` и скрывает карточку.
- Очистка синхронизирует массивы `ringingIntNums`, `ringOrder`, `callShownCards`, `callIdByInt`, а также удаляет временные связи `uniqueidToLinkedid`, чтобы исключить повторные срабатывания.
- В `CallMe.log` можно отследить такие случаи по метке `Forced ring entry cleanup` (указываются `intNum`, `agent_uniqueid`, причина).
- Для ответившего оператора обработчик не срабатывает: записи со статусом `ANSWER` пропускаются, поэтому карточка остаётся активной до завершения разговора.
