...
- **Хранилище состояния.** Все активные звонки ведутся через синглтон `Globals`. В нём хранятся:
  - соответствия `linkedid <-> UniqueID` (`uniqueidToLinkedid`) и `linkedid → CALL_ID`;
  ...
  - отложенные регистрации (`pendingCalls`).
...
- **Привязка LinkedID.** Каждое событие `VarSet` с переменной `CallMeLINKEDID` фиксирует маппинг `UniqueID → linkedid`, поэтому даже каналы очереди (`Local/...`) работают с одним `linkedid`.
...
- Функция `callme_show_card_for_int` показывает карточку конкретному оператору сразу после `DialBegin`; `callme_show_cards_for_ringing` используется как массовый fallback.
